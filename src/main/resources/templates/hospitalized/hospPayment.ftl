...
<div class="c-main pb15 appoin">
    <h4>就诊人：<span id="name">*</span><span id="cardNo">（ 门诊ID号：* ）</span><span id="hiFeeNos" style="display: none"></span></h4>
    ...
    <div class="detailsInfoStay" id="detailsInfoListStay" style="width: 95%; margin: 10px auto;margin-top: 10px; margin-right: auto; margin-bottom: 10px; margin-left: auto;display: none">
        <ul>
...
<script>
    ...
    function details(data) {
        //$('.appolistInfo').style.display ="none";
        document.getElementById("appolistInfo").style.display="none";
        document.getElementById("detailsInfoListStay").style.display="block";
        document.getElementById("hadData").style.display="none";
        ...
        //$('.detailsInfo ul').style.display ="block";
        $('.detailsInfoStay ul').append("<li class=\"islink\">\n"+ "<div class=\"c-list-mess\">\n" +"<div style=\"height: 40px;line-height:40px;display: flex;flex-direction: row; border-bottom: #eeeeee 1px solid;\">\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145 ;width: 50%;'>" +"患者名称"+":" + data.patientName+ "</span>\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145; width: 50%; text-align: right '>" +"缴费编号"+":"+ data.hiFeeNo + "</span>\n" +"</div>\n" +"<div style=\"height: 40px;line-height:40px;display: flex;flex-direction: row; border-bottom: #eeeeee 1px solid;\">\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;'>" +"患者编号"+":"+ data.patientNo + "</span>\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145; width: 50%;text-align: right'>" +"收费员名称"+":"+ data.organdoctorId+ "</span>\n" +"</div>\n" +"<div style=\"height: 40px;line-height:40px;display: flex;flex-direction: row; border-bottom: #eeeeee 1px solid;\">\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;'>" +"挂号医生名称"+":"+ data.doctorName +"</span>\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;text-align: right '>" +"就诊科室名"+":"+ data.organName +"</span>\n" +"</div>\n" +"<div style=\"height: 40px;line-height:40px;display: flex;flex-direction: row; border-bottom: #eeeeee 1px solid;\">\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;'>" +"门诊流水号"+":"+ data.serialNumber +"</span>\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;text-align: right '>" +"打印状态"+":"+ isPrint +"</span>\n" +"</div>\n" +"<div style=\"height: 40px;line-height:40px;display: flex;flex-direction: row; border-bottom: #eeeeee 1px solid;\">\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;'>" +"结算金额"+":"+ data.settleAmount +"</span>\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;text-align: right'>" +"自费金额"+":"+ data.patientAmount +"</span>\n" +"</div>\n" +"<div style=\"height: 40px;line-height:40px;display: flex;flex-direction: row; border-bottom: #eeeeee 1px solid;\">\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;'>" +"就诊日期"+":"+ data.visitDate  +"</span>\n" +
                " <span class=\"c-nowrap mt5\" style='color: #3d4145;width: 50%;text-align: right'>" +"订单创建时间"+":"+ data.createTime +"</span>\n" +"</div>\n" +"<div style=\"height: 40px;line-height:40px;display: flex;flex-direction: row; border-bottom: #eeeeee 1px solid;\">\n" +
                "</span>\n" +"</div>\n" +"</div>\n" + " </li>")
        $.each(data.hiFeeItem, function (i, item) {
            // var data = JSON.stringify(item);
            // console.log(data);
            // $("#listInfo").append("<li>\n" +
            //         " <p class=\"c-nowrap mt5\">" +"名称"+":"+ item.feeItemName +"</p>\n" +
            //         " <p class=\"c-nowrap mt5\">" +"单价"+":"+ item.feeItemAmount +"</p>\n" +
            //         " <p class=\"c-nowrap mt5\">" +"数量"+":"+ item.feeItemNum +item.feeItemUnit+"</p>\n" +
            //         " <p class=\"c-nowrap mt5\">" +"规格"+":"+ item.feeItemStandard +"</p>\n" +
            //         " <p class=\"c-nowrap mt5\">" +"总价"+":"+ item.feeItemAllAmount +
            //         "            </li>")
            $("#listInfoStay").append("<li style='background: #fff;\n" +
                    "    padding: 10px;\n" +
                    "    margin: 10px 0;\n" +
                    "    position: relative;\n" +
                    "    border: 1px solid #ddd;\n" +
                    "    border-radius: 5px;'>\n" +
                    "                <h5>名称：" + item.feeItemName + "</h5>\n" +
                    "                 <h5>单价：" + item.feeItemAmount + "元</h5>\n" +
                    "                 <h5>数量：" + item.feeItemNum + item.feeItemUnit+"</h5>\n" +
                    "                 <em>规格：" + item.feeItemStandard + "元</em>\n" +
                    "                <h5>总价：" + item.feeItemAllAmount + "</h5>\n" +
                    "            </li>")
        });
    }
